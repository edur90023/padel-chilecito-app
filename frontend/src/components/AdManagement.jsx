    ...
    return (
        <div className="animate-fade-in">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => { setShowForm(!showForm); setEditingAd(null); }} className="bg-green-600 text-white px-4 py-2 rounded-md font-medium hover:bg-green-700 transition">
            ...
            {showForm && (
                <div className="bg-gray-800 p-6 rounded-lg mb-6">
                    <AdForm onAdCreated={handleActionComplete} onClose={() => { setShowForm(false); setEditingAd(null); }} initialData={editingAd} />
                </div>
            )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {ads.map(ad => (
                        <div key={ad._id} className="bg-gray-800 rounded-lg shadow-lg overflow-hidden flex flex-col">
                            {ad.imageUrl && <img src={ad.imageUrl} alt="Anuncio" className="w-full h-48 object-contain bg-gray-900" />}
                            <div className="p-4 flex flex-col flex-grow">
                                ...
                                <div className="flex justify-between items-center mt-auto pt-2 border-t border-gray-700">
                                    ...
                                    <div>
                                        <button onClick={() => handleEdit(ad)} className="bg-blue-600 text-white px-3 py-1 rounded-md text-sm hover:bg-blue-700 mr-2">Editar</button>
                                        <button onClick={() => handleDelete(ad._id)} className="bg-red-600 text-white px-3 py-1 rounded-md text-sm hover:bg-red-700">Eliminar</button>
                                    </div>
                                </div>
                            </div>
                        </div>
